...
- [ ] [debt] `symlink-guard.sh` Case 4: text-form pointer is only safe for POSIX peers when `.agents/skills` entry is already tracked as git mode 120000; warn or guard if not (source: Claude) — `kadragon-tools/skills/harness-init/scripts/symlink-guard.sh:44`
