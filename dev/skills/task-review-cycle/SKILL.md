...
Wait for every launched source before Step 3; a breached one leaves its result on disk — `references/late-source-reclaim.md` reclaims it before merge.
...
**Lite path** — reclaim any skipped source (`references/late-source-reclaim.md`), then merge locally and push `main`:
...
(15 min; `reason:"rework-cap"` and `reason:"timeout"` stop and ask), then reclaim any skipped source after CI green (`references/late-source-reclaim.md`), then:
...
| Reviewer sentinel or >1200s | Inline review, note it in the report; reclaim before merge |
| Panel source fails, exits 75, or >1200s | Record `Reviewers Skipped: <reason>`, proceed, reclaim before merge |
