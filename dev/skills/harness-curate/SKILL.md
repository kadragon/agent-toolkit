...
Past harness edits carry falsifiable predictions (`references/harness-evolution.md` §3 — the loop contract's change record). This step is what falsifies them, using the scan output Step 1 just produced.

1. Resolve `TARGET_REPO` (the `--project` path, or cwd on `current` scope — same resolution as Step 7) and read `<TARGET_REPO>/docs/harness-log.md`. **Absent file = no pending rows — skip this step silently, it is not an error** (a repo with zero loop-originated edits is the normal case). `all` scope skips too: no resolvable repo path (same limitation as Signal 7 and the Codex fold-in).
2. Load change-history rows whose `Verified` is `pending` or `unverified` (`unverified` rows first — they landed without a check).
3. Judge each `Predicted impact` against this run's evidence — Step 1's scan blocks (`SKILLS-ACTIVE`, `CORRECTION-SIGNALS`, `VERIFIER-FAILURES`, …) and, where the prediction names one, a direct read/command. Only an observation made this run counts as evidence; if the prediction's window has not elapsed or the evidence is not in view, leave the row untouched.
...
5. **Failed** (the predicted change observably did not happen, or the mistake recurred) → do not stamp; write `failed` and surface the edit as a **prune/rework candidate** row in the Step 6 report. Any resulting delete goes through the Step 7 adversarial check like every other demote.
