...
| Underperforming asset (telemetry) | 1 `declining` verdict — it is a trend, not an instance — but only above **all three** minimum run counts (default: 3 in the 7d window, 10 in the 30d, and 3 older than the 7d window — a skill with no history outside the recent window compares against itself); `insufficient-data` is never a finding and never a `Watch:` row |
