...
| `.claude/agents/{role}.md` | YAML frontmatter (`name`, `description`, `model`; `tools` optional) + body with Objective / Spawn Prompt Contract / Effort Tier / Exit Criteria | init Step 4c (if multi-agent) | `validate-harness.sh` §11 (presence only — WARN) |
