...
3. Apply these selection rules:
   ...
   - **Always include** one general-purpose code reviewer if available. Priority order: `review` > `pr-review-toolkit:review-pr`. Prefer `review` — `pr-review-toolkit:review-pr` spawns named sub-agents (code-reviewer, test-reviewer, error-hunter) that are not cleaned up on completion and leave zombie agents in the session. Use `pr-review-toolkit:review-pr` only as fallback when `review` is not in `review_candidates`. Both are git-diff-based and work on any host (GitHub, Forgejo, etc.). Note: `code-review` (kind=command) uses `gh pr` commands internally and requires a GitHub PR to exist — skip it when `--no-hub` is set, when `HUB_TYPE` is not `github`, or when no PR has been created yet.
