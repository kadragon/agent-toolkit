...
If no P0/P1 in-scope suggestions exist — even if P2/P3 items were found and routed to backlog — skip Steps 4–5 (apply fixes and commit). Step 6 (merge/push) still executes unless `--no-hub` is set, in which case the workflow ends after this step.
