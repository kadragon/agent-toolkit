...
3. Apply these selection rules:
   ...
   - **Always include** one general-purpose code reviewer if available. Use `review` (builtin) — do NOT use `pr-review-toolkit:review-pr`; it spawns named sub-agents (code-reviewer, test-reviewer, error-hunter) that are not cleaned up on completion and leave zombie agents in the session. Note: `code-review` (kind=command) uses `gh pr` commands internally and requires a GitHub PR to exist — skip it when `--no-hub` is set, when `HUB_TYPE` is not `github`, or when no PR has been created yet.
