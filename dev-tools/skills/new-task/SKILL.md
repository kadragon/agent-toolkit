...
described that is not yet an item in `backlog.md`/`tasks.md`. Sibling to `next-tasks`: that skill
...
Boundary vs `next-tasks`: if the request is already a `backlog.md`/`tasks.md` item, stop and use
`next-tasks` — do not re-enter it here. This skill runs when the request has no tracking entry yet.
...
**Trivial** → skip grill/`to-spec`/`to-tickets` entirely. Build the Sprint Contract directly from
...
the request) → `Skill(dev-tools:grill)` to resolve scope. Do not proceed until grill reports the
open questions resolved.

**After resolution, judge size:**

- **Single-session-sized** → build the Sprint Contract directly from the grill output (or directly
  from the request, if grill was skipped because it was unambiguous but not trivial) and go to
  Step 3.
- **Multi-session or architecturally significant** → `Skill(dev-tools:to-spec)` to write
  `docs/design/{slug}.md`, then `Skill(dev-tools:to-tickets)` to break the approved spec into
  ordered `backlog.md` items. Once `to-tickets` has written the tickets, **pick the first
  ...
  `next-tasks` runs — do NOT try to implement more than one ticket in this invocation.
...
- If this cycle is running a `to-tickets`-generated backlog ticket (the multi-session path), add a
...
**Do NOT commit.** Leave everything uncommitted — `dev-review-cycle` Step 1 makes the single commit.
...
Invoke `Skill(dev-tools:dev-review-cycle)` with `args: --auto`. It commits (including the cleanup
...
If `dev-review-cycle` reports CI failure and the PR must be abandoned: close the PR and delete the
...
- User picks **1** → run Step 3 (branch, Sprint Contract, implement, QA, version bump, cleanup) then
  skip `dev-review-cycle` and merge directly:
...
```
  ...
  **Branch-protection caveat:** if `git push origin main` is rejected (PR-only rule), reset local
  ...
  `Skill(dev-tools:dev-review-cycle)` with `args: --auto`.
...
item, stop and route the user to `next-tasks` instead of duplicating the entry.
...
`backlog.md` and point at `next-tasks --all`).

**Work already in flight** — if `git status` shows an in-flight feature branch rather than a fresh
request, this is `next-tasks` territory (its "Work already in flight" edge case). Route there.
