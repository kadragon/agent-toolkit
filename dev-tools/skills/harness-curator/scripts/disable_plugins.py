...
def run_tests() -> int:
    ...
    global_enabled = {
        ...
        "productivity@kadragon": False,   # already false — should be skipped
        "other-tool@vendor": True,
    }

    # ---- Test 1: basic resolution ----
    print("Test 1: resolve_enabled_keys — basic hit")
    result = resolve_enabled_keys(global_enabled, ["dev-tools"])
    _assert(result == {"dev-tools": "dev-tools@kadragon"}, "dev-tools resolves correctly")

    # ---- Test 2: already-false skip ----
    print("\nTest 2: resolve_enabled_keys — already-false skip")
    result = resolve_enabled_keys(global_enabled, ["productivity"])
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        write_disabled(proj_settings, ["dev-tools@kadragon"])
        with open(proj_settings) as fh:
            data = json.load(fh)
        _assert(data.get("enabledPlugins", {}).get("dev-tools@kadragon") is False,
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        _assert(data["enabledPlugins"].get("dev-tools@kadragon") is False,
