...
#   protection_action: created | already_present | skipped
...
gh api -X PATCH "repos/$repo" -F allow_auto_merge=true --silent || {
  echo "ERROR: failed to enable allow_auto_merge on $repo" >&2
  exit 1
}
...
echo "$protection_payload" | gh api -X PUT "repos/$repo/branches/$default_branch/protection" \
