...
- [ ] [FIX] commit-guard newline/`;`-separator false-block — `_split_segments` (guard.py:128) normalizes unquoted newlines to `;`, but in-chain branch-creation state (`git checkout -b X`) only carries across `&&` (docstring: "only '&&' guarantees the predecessor ran successfully"). So a common multi-line script — `git checkout -b X` on one line, `git commit` on a later line (shell-equivalent to `;`) — drops the branch-switch, falls back to the live branch (`main`), and false-blocks a legitimate commit. Empirically hit during PR #111 follow-up (worked around by splitting into separate Bash calls). Consider carrying a *successful-looking* `checkout -b`/`switch -c` across `;`/newline too (a bare `checkout -b` rarely fails), or only block when NO branch-switch appears in ANY prior segment regardless of operator. Preserve the fail-toward-block contract for genuinely ambiguous chains. Bump dev-tools manifests on change.
