...
1. **Version bump mandatory** — If files under `dev-tools/` changed, both `dev-tools/.claude-plugin/plugin.json` AND `dev-tools/.codex-plugin/plugin.json` versions must increment (keep in sync). Same for `productivity/`. Enforced by CI (`harness-check.yml`) for Claude; Codex bump is manual. Semver: add skill/agent → minor; modify → patch; remove/rename → major.
